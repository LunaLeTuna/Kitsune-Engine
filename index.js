...
awaowo.position = new Vector3(0,9,0);
awaowo.rotation = new Vector3(90,0,0);
SetMainCam(awaowo);
...
function tick(delta){ //called every frame (TODO: multi thread)
    ...
    awaowo.position = new Vector3(bbbb.position.x, 15, bbbb.position.z);

    fps.text = "volocity | x: "+bbbb.LinearVelocity.x+" | y: "+bbbb.LinearVelocity.z+"\nbox | x: "+bbbb.position.x+" | y: "+bbbb.position.z+"\ntry talking to her, don't be lazy, remeber no logic";

    if(bbbb.LinearVelocity.x > maxspeed){
        bbbb.LinearVelocity = new Vector3(maxspeed, bbbb.LinearVelocity.y, bbbb.LinearVelocity.z);
    }
    ...
    if(bbbb.LinearVelocity.z > maxspeed){
    ...
    bbbb.AddForce(new Vector3(xsa,0,ysa));
